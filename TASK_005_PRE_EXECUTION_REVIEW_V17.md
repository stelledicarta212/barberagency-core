...
| G | rol local inexistente | mock `pg_roles` sin rol | catálogo mock | role validator | FUTURE_CONTROLLED_DATABASE_TEST | isolated temp | excepción | aborta | `database_acl_policy_role_missing` | 1 | CONTROLLED_TEMP_RESTORE_POLICY_VALIDATED | catálogo controlado | rol presente | resultado SQL | YES | YES | NO | NO | COMPLETED_WITH_EVIDENCE |
...
| N | schema inesperado | discovered `extra` | catálogo mock | schema comparator | FUTURE_CONTROLLED_DATABASE_TEST | isolated temp | excepción | aborta | `database_acl_policy_schema_unexpected` | 1 | TEMP_RESTORE_TOC_ALLOWLIST_VALIDATED | set diff | set igual | resultado SQL | YES | YES | NO | NO | COMPLETED_WITH_EVIDENCE |
...
| P | rol administrativo | `rolsuper=true` | catálogo mock | role validator | FUTURE_CONTROLLED_DATABASE_TEST | isolated temp | excepción | aborta | `database_acl_policy_role_admin_attribute` | 1 | CONTROLLED_TEMP_RESTORE_POLICY_VALIDATED | cada atributo | rol NO* | resultado SQL | YES | YES | NO | NO | COMPLETED_WITH_EVIDENCE |
| Q | membresía admin directa/indirecta | graph `val→admin` | catálogo mock con ciclo | membership validator | FUTURE_CONTROLLED_DATABASE_TEST | isolated temp | excepción | aborta | `database_acl_policy_indirect_membership_forbidden` | 1 | CONTROLLED_TEMP_RESTORE_POLICY_VALIDATED | recursión visitada | sin membresía | resultado SQL | YES | YES | NO | NO | COMPLETED_WITH_EVIDENCE |
| R | EXECUTE fuera de allowlist | grant execute | allowlist vacía | ACL assertion | FUTURE_CONTROLLED_DATABASE_TEST | isolated temp | excepción | aborta | `database_acl_policy_execute_privilege` | 1 | ACL_PRIVILEGE_CONTENT_VALIDATED | función mock | no execute | resultado SQL | YES | YES | NO | NO | COMPLETED_WITH_EVIDENCE |
| S | SECURITY DEFINER accesible | `prosecdef=true` + execute | catálogo mock | proc assertion | FUTURE_CONTROLLED_DATABASE_TEST | isolated temp | excepción | aborta | `database_acl_policy_execute_privilege` | 1 | ACL_PRIVILEGE_CONTENT_VALIDATED | secdef exacto | invoker sin execute | resultado SQL | YES | YES | NO | NO | COMPLETED_WITH_EVIDENCE |
| T | escritura inesperada | grant INSERT/UPDATE | catálogo mock | ACL assertion | FUTURE_CONTROLLED_DATABASE_TEST | isolated temp | excepción | aborta | `database_acl_policy_write_privilege` | 1 | ACL_PRIVILEGE_CONTENT_VALIDATED | cada write priv | solo SELECT | resultado SQL | YES | YES | NO | NO | COMPLETED_WITH_EVIDENCE |
| U | privilegio PUBLIC con ACL NULL | acl null + default PUBLIC | catálogo mock | ACL semantics | FUTURE_CONTROLLED_DATABASE_TEST | isolated temp | excepción | aborta | `database_acl_policy_public_privilege` | 1 | ACL_PRIVILEGE_CONTENT_VALIDATED | `acldefault` | no PUBLIC | resultado SQL | YES | YES | NO | NO | COMPLETED_WITH_EVIDENCE |
| V | error antes de COMMIT exige rollback | raise before commit | SQL mock | transaction policy | FUTURE_CONTROLLED_DATABASE_TEST | isolated temp | rollback | no cambios | `database_acl_policy_transaction_failed` | 1 | CONTROLLED_TEMP_RESTORE_POLICY_VALIDATED | estado pre/post | commit válido | resultado SQL | YES | YES | NO | NO | COMPLETED_WITH_EVIDENCE |
...
### 12.4.2 Cierre documental de TASK-005 — 2026-07-29

El Owner aprobo Gate H y autorizo el cierre documental de TASK-005 con alcance limitado. La autorizacion no incluye produccion, EasyPanel, PostgreSQL externo o productivo, datos reales, migraciones productivas, despliegues, credenciales, archivos `.env`, cambios productivos de RLS/permisos, R2 real, n8n externo, Mercado Pago, pagos reales, merge, PR ni TASK-007.
...
Correccion documental del hash fisico:
...
TASK-006 no fue ejecutada porque no se encontro una definicion canonica versionada inequivoca en el worktree auditado.
...
## 14. TASK_005_B7_EXIT_CODE_EXTERNAL_CAPTURE_CORRECTION

```text
CORRECTION_SCOPE = B7_EXIT_CODE_EXTERNAL_CAPTURE_ONLY
PREVIOUS_REVIEW = CHANGES_REQUIRED
PREVIOUS_REVIEWED_COMMIT = cba39102a77917d989d78c00932be4c4afdb7799
PREVIOUS_BLOCKING_CAUSE = POWERSHELL_REAL_EXIT_CODE_ABSENT_FROM_VERSIONED_LOGS
BASE_HEAD_CORRECTION = cba39102a77917d989d78c00932be4c4afdb7799
CORRECTION_METHOD = VERSIONED_EXTERNAL_WRAPPER
EXTERNAL_WRAPPER = pruebas/task005_b7_runner.ps1
RUN_LOG_MANUAL_EDITING = NO
RUN_LOGS_REGENERATED_FROM_SCRATCH = YES
HARNESS_DYNAMIC_EXECUTIONS = 3
PRODUCTION_ACCESSED = NO
EASYPANEL_ACCESSED = NO
POSTGRESQL_EXTERNAL_ACCESSED = NO
SECRETS_READ = NO
...
La auditoria independiente anterior dejo B7 en `CHANGES_REQUIRED` exclusivamente por ausencia de captura externa versionada de `POWERSHELL_REAL_EXIT_CODE` en `pruebas/task005_b7_evidence_run1.log`, `pruebas/task005_b7_evidence_run2.log` y `pruebas/task005_b7_evidence_run3.log`.

La correccion implementa un wrapper externo reproducible (`pruebas/task005_b7_runner.ps1`) que ejecuta `pruebas/task005_b7_harness.ps1` como proceso PowerShell hijo independiente para `RUN_SEQUENCE` 1, 2 y 3. El wrapper reconstruye desde cero cada log, captura el exit code real del proceso hijo despues de su finalizacion y agrega automaticamente `POWERSHELL_REAL_EXIT_CODE: 0` solo cuando coincide con `HARNESS_GLOBAL_EXIT_CODE: 0`.

### 14.1 Evidencia regenerada

| RUN | RUN_ID | RUN_STARTED_AT_UTC | RUN_FINISHED_AT_UTC | HARNESS_GLOBAL_EXIT_CODE | POWERSHELL_REAL_EXIT_CODE | CLEANUP_STATUS | SHA-256 | Size bytes |
|---|---|---|---|---:|---:|---|---|---:|
| RUN_1 | `26220c9a-acd3-4126-8408-4c31cc0d3f67` | `2026-07-29T01:40:34.9603547Z` | `2026-07-29T01:40:46.5240200Z` | 0 | 0 | SUCCESS | `C5AB7907465F8610325A57644C290AB5F6D4B25B713B7DD93798DD257A3EA614` | 3372 |
| RUN_2 | `2f3a5bd2-5189-470d-a93c-4155af8ce716` | `2026-07-29T01:40:46.8071062Z` | `2026-07-29T01:40:58.6286351Z` | 0 | 0 | SUCCESS | `62FFF55E475DE0D55DA2207798A5FBA19F08694E52627ABD791C58BAC8BB1883` | 3372 |
| RUN_3 | `b762f21b-067f-436f-9ad8-ac24e2bb6eba` | `2026-07-29T01:40:58.8854775Z` | `2026-07-29T01:41:10.7333367Z` | 0 | 0 | SUCCESS | `2B46FA5B3E8B67A7903FB4550460E75E00DC7A1CB48B5974ED84DFB19D046C8F` | 3372 |

### 14.2 Validaciones registradas

```text
RUN_1_REAL_EXECUTION = YES
RUN_2_REAL_EXECUTION = YES
RUN_3_REAL_EXECUTION = YES
GLOBAL_EXTERNAL_EXIT_CODES_MATCH = YES
THREE_RUNS_VERIFIED = YES
RUN_IDS_UNIQUE = YES
TIMESTAMPS_COHERENT = YES
HASHES_UNIQUE = YES
R_S_T_V_REGRESSION = PASS
U_STATUS = PASS
DELIBERATE_MISMATCH_DOCUMENTED = YES
ASSERTIONS_BLOCKING = PASS
B1_B6_REGRESSION = PASS_STATIC
EVIDENCE_SANITIZED = YES
CLEANUP = SUCCESS
```

### 14.3 Estados contractuales preservados
...
## 16. TASK_005_GATE_D_STATIC_PARSER_VALIDATION_CLOSURE
...
GATE_D_STATUS = CLOSED
GATE_D_EXECUTED_AT_UTC = 2026-07-29T13:59:18.8938529Z
GATE_D_COMPLETED_AT_UTC = 2026-07-29T13:59:20.2346868Z
