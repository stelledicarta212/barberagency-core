...
async function setup() {
    ...
    nodes: [
      ...
      },
      {
        parameters: {
          ...
          options: {
            queryReplacement: "={{ [Number($json.user_id), Number($json.barberia_id), $json.action, Number($json.servicio_id), Boolean($json.auth_ok)] }}"
          }
        },
        id: "session-validate",
  ...
  console.log('Activating test workflow...');
...
async function cleanup() {
  if (tempWorkflowId) {
    console.log('Deactivating test workflow...');
...
(async () => {
  try {
    await setup();
    console.log('Calling test webhook without cookie...');
