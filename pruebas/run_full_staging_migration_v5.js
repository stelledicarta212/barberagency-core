...
function callWebhook(body) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      hostname: 'barberagency-n8n.gymh5g.easypanel.host',
      port: 443,
      path: '/webhook/temp_postgres_exec_v5',
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function setup() {
  const workflowDefinition = {
    name: "Temp Postgres Exec Webhook V5",
    nodes: [
      {
        parameters: {
          httpMethod: "POST",
          path: "temp_postgres_exec_v5",
  ...
  console.log('Creating temp workflow V5...');
  ...
  console.log('Activating temp workflow V5...');
  await req('POST', `/api/v1/workflows/${tempWorkflowId}/activate`);
  console.log('Workflow active.');
  
  console.log('Waiting 2 seconds for webhook endpoint propagation...');
  await new Promise(r => setTimeout(r, 2000));
}

async function cleanup() {
  if (tempWorkflowId) {
    console.log('Deactivating temp workflow V5...');
    await req('POST', `/api/v1/workflows/${tempWorkflowId}/deactivate`).catch(() => {});
    console.log('Deleting temp workflow V5...');
...
async function runSQL(query) {
  ...
  // Check for nested error objects inside HTTP 200
  if (res.data && res.data[0] && res.data[0].error) {
    throw new Error(`PostgreSQL execution error: ${res.data[0].message || JSON.stringify(res.data[0].error)}`);
  }
  return res.data;
}
...
(async () => {
  try {
    await setup();
    console.log('\n========================================');
    console.log('STARTING STAGING MIGRATION PIPELINE V5');
    console.log('========================================\n');
