...
(async () => {
  try {
    await setup();
    console.log('--- UPDATING FUNCTION: ba_publicar_barberia ---');
    ...
    await runSQL(`
      ...
      BEGIN
        ...
        SELECT email INTO v_auth_email FROM public.usuarios WHERE id = v_auth_user_id;
        ...
        -- 4. Validar pertenencia del dueño o miembro admin activo
        IF NOT (
          (v_barberia.owner_id = v_auth_user_id) OR
          EXISTS (
            SELECT 1 FROM public.barberia_miembros m 
            WHERE m.barberia_id = p_barberia_id 
              AND (m.usuario_id = v_auth_user_id OR lower(m.email) = lower(v_auth_email)) 
