...
-- 0. Create ba_public_reserva_error helper function if not exists
...
-- 1. Create tiene_acceso helper function
CREATE OR REPLACE FUNCTION public.tiene_acceso(p_barberia_id integer)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  RETURN EXISTS (
    ...
    WHERE s.barberia_id = p_barberia_id
      AND (s.status = 'active' OR s.estado = 'activa')
      AND (s.period_end IS NULL OR s.period_end > now())
  );
END;
$$;

-- 2. Update ba_reservas_public_create to enforce tiene_acceso
...
BEGIN
  ...
  -- ENFORCE ENTITLEMENT CHECK (tiene_acceso)
  IF NOT public.tiene_acceso(v_barberia.id) THEN
    RETURN public.ba_public_reserva_error('BOOKING_ACCESS_DISABLED', 'Acceso deshabilitado. La barberia no cuenta con suscripcion activa.');
  END IF;
...
REVOKE ALL ON FUNCTION public.tiene_acceso(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.tiene_acceso(integer) TO anon, authenticated;
